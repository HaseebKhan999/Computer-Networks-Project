...
const TrafficAnalyzer = () => {
    ...
    return (
        <div className="p-4 bg-card rounded-lg shadow-xl ml-3 mt-4 max-w-full border border-border">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-extrabold mr-3 text-foreground">AI Traffic Insight</h3>
                <button 
            ...
            {loading && (
                <div className="bg-secondary p-6 rounded-xl border border-border flex items-center justify-center">
                    <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-teal-500 mr-3"></div>
                    <span className="text-muted-foreground">Gemini is synthesizing network intelligence...</span>
                </div>
            )}
            
            {/* Single Summary Rendering */}
            {contentString && !loading && (
                <div className="bg-secondary p-4 rounded-xl border border-border text-foreground text-base">
                    <p>
                        <span className="font-bold text-blue-600 dark:text-blue-400 mr-2">Primary Activity:</span>
            ...
            {!loading && summary && !contentString && (
                 <div className="bg-secondary p-4 rounded-xl border border-border text-red-600 dark:text-red-400 whitespace-pre-line">
